...
# ACFの計算（自己相関）、入力形式はnumpy、出力はテンソル
def culc_acf(image):

    # ACFの計算 (自己相関)
    acf_result = signal.correlate(image, image, mode='same')
    acf_tensor = torch.tensor(acf_result)  # numpyからテンソルへ変換
...
def train_load_data(cache_path, real_dir_path, pggan_dir_path, stargan_dir_path, stylegan_dir_path, target_size=(224, 224)):
    ...
    for img_path in img_path_list:
        ...
        # 画像特徴をセット
        acf_features.append(culc_acf(img))
    ...
    acf_features = torch.stack(acf_features)  # リストからテンソルに変換
    labels = torch.tensor(labels)
    
    # 画像データ・正解ラベルのペアをデータにセットする
    dataset = torch.utils.data.TensorDataset(acf_features, labels)
...
def cnn_train(net, device, loaders, data_size, optimizer, e, history):
    ...
    for i, (acf_data, labels) in tqdm(enumerate(loaders)):
        # GPUあるいはCPU用に再構成
        acf_data = acf_data.to(device)      # バッチサイズのデータtensor
    ...
    # フォルダがなければ作る
    os.makedirs("./Results/ACF", exist_ok=True)

    # 結果保存
    with open("./Results/ACF/ACF_style_train.txt", "w") as ftw:
...
def cnn_validation(net, device, loaders, data_size, e, epoch, history):
    ...
    with torch.no_grad():
        for i, (acf_data, labels) in enumerate(loaders):
            # GPUあるいはCPU用に再構成
            acf_data = acf_data.to(device)      # バッチサイズのデータtensor
            labels = labels.to(device)  # バッチサイズの正解ラベルtensor
    ...
    # 結果保存
    with open("./Results/ACF/ACF_style_validation.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./ACF_style__NG_photo_CNN"
...
def cnn_test(net, device, model_path, loaders, data_size):
    ...
    # 結果保存
    with open("./Results/ACF/ACF_style_test.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./NG_photo_CNN/ACF_style_"
    os.makedirs(dir_path, exist_ok=True)
...
def output_graph(epoch, history_train, history_test):
    os.makedirs("./CNNLearningResult/ACF_style_", exist_ok=True)
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/ACF_style_/loss_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/ACF_style_/acc_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/ACF_style_/f1_cnn.png')
...
def Main(args):
    ...
    train_loaders, train_data_size = train_load_data("D:/GraduationResearch/Features/ACF_style_train_cache.pt", ffhq_train_dir, pggan_train_dir, star_train_dir, style_train_dir)
    validation_loaders, validation_data_size = test_load_data("D:/GraduationResearch/Features/ACF_style_validation_cache.pt", ffhq_validation_dir, pggan_validation_dir, star_validation_dir, style_validation_dir)
    ...
    # 学習済みパラメータを保存
    torch.save(net.state_dict(), './params/ACF_style_params_cnn.pth')
    ...
    test_loaders, test_data_size = test_load_data("D:/GraduationResearch/Features/ACF_style_test_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir, style_test_dir)
    ...
    cnn_test(testnet, device, "./params/ACF_style_params_cnn.pth", test_loaders, test_data_size)
