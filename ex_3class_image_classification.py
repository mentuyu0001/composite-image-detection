...
def cnn_classification_test(net, device, ffhq_test_dir, pggan_test_dir, star_test_dir1, star_test_dir2,  style_test_dir, faceapp_test_dir, history):
    ...
    loaders, size = test_load_realdata("D:/GraduationResearch/Features/test_real_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir1, style_test_dir, faceapp_test_dir)
    ...
    # 結果保存
    with open("./Results/FinalResults/3class_image_classification.txt", "w") as ftw:
        ftw.write("\nClassification Real Images Score\n")
    ...
    # pggan画像の処理
    loaders, size = test_load_pggandata("D:/GraduationResearch/Features/test_pg_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir1, style_test_dir, faceapp_test_dir)
    ...
    # 結果保存
    with open("./Results/FinalResults/3class_image_classification.txt", "a") as ftw:
        ftw.write("\nClassification PGGAN Score\n")
        ftw.write(f"Train correct / total : {str(test_correct_counter)} / {str(size)}\n")
        ftw.write(f"Train Accuracy: {str(test_correct_counter / size)}\n")
        ftw.write(f"Train F1 Score: {str(f1)}\n")
        ftw.write(f"Train Cross-Entropy Loss: {str(total_loss / size)}\n")
    ...
    loaders, size = test_load_stargandata("D:/GraduationResearch/Features/test_star1_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir1, style_test_dir, faceapp_test_dir)
    ...
    star_size += size
    star_correct += test_correct_counter
    star_loss += total_loss
    star_f1 += f1 * size


    # star2画像の処理
    loaders, size = test_load_stargandata("D:/GraduationResearch/Features/test_star2_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir2, style_test_dir, faceapp_test_dir)
    ...
    # 結果保存
    with open("./Results/FinalResults/3class_image_classification.txt", "a") as ftw:
        ftw.write("\nClassification StarGAN Score\n")
        ftw.write(f"Train correct / total : {str(star_correct)} / {str(star_size)}\n")
        ftw.write(f"Train Accuracy: {str(star_correct / star_size)}\n")
        ftw.write(f"Train F1 Score: {str(star_f1 / star_size)}\n")
        ftw.write(f"Train Cross-Entropy Loss: {str(star_loss / star_size)}\n")

    # style画像の処理
    loaders, size = test_load_stylegandata("D:/GraduationResearch/Features/test_style_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir1, style_test_dir, faceapp_test_dir)
    ...
    # faceapp画像の処理
    loaders, size = test_load_faceappdata("D:/GraduationResearch/Features/test_faceapp_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir1, style_test_dir, faceapp_test_dir)
    ...
    # 結果を出力
    print(f"correct / total: {final_test_corect_counter} / {final_data_num}")
    print(f"Test Accuracy: {final_test_corect_counter / final_data_num}")
    print(f"F1 Score: {final_f1 / final_data_num}")
    print(f"Average Cross-Entropy Loss: {final_total_loss / final_data_num}")
...
def imgs_output(imgs, test_pred, target, counter, batch_size):
    ...
    # フォルダがなければ作る.
    # ある場合は中身を消去して再生成する
    dir_RealToPg = "./ImageRealToPg"
    dir_RealToStar = "./ImageRealToStar"
    dir_RealToStyle = "./ImageRealToStyle"
    dir_PgToPg = "./ImagePgToPg"
    dir_PgToStar = "./ImagePgToStar"
    dir_PgToStyle = "./ImagePgToStyle"
    dir_StarToPg = "./ImageStarToPg"
    dir_StarToStar = "./ImageStarToStar"
    dir_StarToStyle = "./ImageStarToStyle"
    dir_StyleToPg = "./ImageStyleToPg"
    dir_StyleToStar = "./ImageStyleToStar"
    dir_StyleToStyle = "./ImageStyleToStyle"
    dir_FaceToPg = "./ImageFaceToPg"
    dir_FaceToStar = "./ImageFaceToStar"
    dir_FaceToStyle = "./ImageFaceToStyle"
...
def Main(args):
    ...
    dir_FaceToStyle = "./ImageFaceToStyle"
    # フォルダーが存在する場合、中身を削除
    if os.path.exists(dir_RealToPg):
        shutil.rmtree(dir_RealToPg)
    if os.path.exists(dir_RealToStar):
        shutil.rmtree(dir_RealToStar)
    if os.path.exists(dir_RealToStyle):
        shutil.rmtree(dir_RealToStyle)
    if os.path.exists(dir_PgToPg):
        shutil.rmtree(dir_PgToPg)
    if os.path.exists(dir_PgToStar):
        shutil.rmtree(dir_PgToStar)
    if os.path.exists(dir_PgToStyle):
        shutil.rmtree(dir_PgToStyle)
    if os.path.exists(dir_StarToPg):
        shutil.rmtree(dir_StarToPg)
    if os.path.exists(dir_StarToStar):
        shutil.rmtree(dir_StarToStar)
    if os.path.exists(dir_StarToStyle):
        shutil.rmtree(dir_StarToStyle)
    if os.path.exists(dir_StyleToPg):
        shutil.rmtree(dir_StyleToPg)
    if os.path.exists(dir_StyleToStar):
        shutil.rmtree(dir_StyleToStar)
    if os.path.exists(dir_StyleToStyle):
        shutil.rmtree(dir_StyleToStyle)
    if os.path.exists(dir_FaceToPg):
        shutil.rmtree(dir_FaceToPg)
    if os.path.exists(dir_FaceToStar):
        shutil.rmtree(dir_FaceToStar)
    if os.path.exists(dir_FaceToStyle):
        shutil.rmtree(dir_FaceToStyle)
    # フォルダーを再作成
    os.makedirs(dir_RealToPg, exist_ok=True)
    os.makedirs(dir_RealToStar, exist_ok=True)
    os.makedirs(dir_RealToStyle, exist_ok=True)
    os.makedirs(dir_PgToPg, exist_ok=True)
    os.makedirs(dir_PgToStar, exist_ok=True)
    os.makedirs(dir_PgToStyle, exist_ok=True)
    os.makedirs(dir_StarToPg, exist_ok=True)
    os.makedirs(dir_StarToStar, exist_ok=True)
    os.makedirs(dir_StarToStyle, exist_ok=True)
    os.makedirs(dir_StyleToPg, exist_ok=True)
    os.makedirs(dir_StyleToStar, exist_ok=True)
    os.makedirs(dir_StyleToStyle, exist_ok=True)
    os.makedirs(dir_FaceToPg, exist_ok=True)
    os.makedirs(dir_FaceToStar, exist_ok=True)
    os.makedirs(dir_FaceToStyle, exist_ok=True)
    ...
    net.load_state_dict(torch.load("params/IMAGE_params_cnn.pth", map_location=device, weights_only=False))
    net.to(device)
    net.eval()

    cnn_classification_test(net, device, ffhq_test_dir, pggan_test_dir, star_test_dir1, star_test_dir2, style_test_dir, faceapp_test_dir, history_test)
