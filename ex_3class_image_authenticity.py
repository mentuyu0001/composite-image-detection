...
def authenticityTest(pgnet, starnet, stylenet, device, dir_Pg, dir_Star, dir_Style, final_test_corect_counter, final_data_num, final_total_loss, final_f1, kind):
    #初期化達
    realcorrect = 0
    realsize = 0
    realloss = 0
...
def cnn_classification_test(pgnet, starnet, stylenet, device, 
    ...
    #Real
    final_test_corect_counter, final_data_num, final_total_loss, final_f1 = authenticityTest(pgnet, starnet, stylenet, device,
                                        dir_RealToPg, dir_RealToStar, dir_RealToStyle, 
                                        final_test_corect_counter, final_data_num, final_total_loss, final_f1, 0)
    
    #PG
    final_test_corect_counter, final_data_num, final_total_loss, final_f1 = authenticityTest(pgnet, starnet, stylenet, device,
                                        dir_PgToPg, dir_PgToStar, dir_PgToStyle, 
                                        final_test_corect_counter, final_data_num, final_total_loss, final_f1, 1)
    
    #Real
    final_test_corect_counter, final_data_num, final_total_loss, final_f1 = authenticityTest(pgnet, starnet, stylenet, device,
                                        dir_StarToPg, dir_StarToStar, dir_StarToStyle, 
                                        final_test_corect_counter, final_data_num, final_total_loss, final_f1, 2)
    
    #Real
    final_test_corect_counter, final_data_num, final_total_loss, final_f1 = authenticityTest(pgnet, starnet, stylenet, device,
                                        dir_StyleToPg, dir_StyleToStar, dir_StyleToStyle, 
                                        final_test_corect_counter, final_data_num, final_total_loss, final_f1, 3)
    
    #Real
    final_test_corect_counter, final_data_num, final_total_loss, final_f1 = authenticityTest(pgnet, starnet, stylenet, device,
                                        dir_FaceToPg, dir_FaceToStar, dir_FaceToStyle, 
                                        final_test_corect_counter, final_data_num, final_total_loss, final_f1, 4)
...
def Main(args):
    ...
    #data
    dir_RealToPg = args.realtopgdataimg
    dir_RealToStar = args.realtostardataimg
    dir_RealToStyle = args.realtostyledataimg
    dir_PgToPg = args.pgtopgdataimg
    dir_PgToStar = args.pgtostardataimg
    dir_PgToStyle = args.pgtostyledataimg
    dir_StarToPg = args.startopgdataimg
    dir_StarToStar = args.startostardataimg
    dir_StarToStyle = args.startostyledataimg
    dir_StyleToPg = args.styletopgdataimg
    dir_StyleToStar = args.styletostardataimg
    dir_StyleToStyle = args.styletostyledataimg
    dir_FaceToPg = args.facetopgdataimg
    dir_FaceToStar = args.facetostardataimg
    dir_FaceToStyle = args.facetostyledataimg
    ...
    # 3つのデータを使うので、その分用意する
    pgnet : torch.nn.Module = MyNet()
    starnet : torch.nn.Module = MyNet()
    stylenet : torch.nn.Module = MyNet()

    #モデルのインスタンス化とパラメータの読み込み
    pgnet.load_state_dict(torch.load("params/IMAGE_pg_params_cnn.pth", map_location=device, weights_only=False))
    pgnet.to(device)
    pgnet.eval()

    starnet.load_state_dict(torch.load("params/IMAGE_star_params_cnn.pth", map_location=device, weights_only=False))
    starnet.to(device)
    starnet.eval()

    stylenet.load_state_dict(torch.load("params/IMAGE_style_params_cnn.pth", map_location=device, weights_only=False))
    stylenet.to(device)
    stylenet.eval()


    cnn_classification_test(pgnet, starnet, stylenet, device, 
                            dir_RealToPg, dir_RealToStar, dir_RealToStyle,
                            dir_PgToPg, dir_PgToStar, dir_PgToStyle,
                            dir_StarToPg, dir_StarToStar, dir_StarToStyle,
                            dir_StyleToPg, dir_StyleToStar, dir_StyleToStyle,
