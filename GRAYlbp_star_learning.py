...
def cnn_train(net, device, loaders, data_size, optimizer, e, history):
    ...
    # 結果保存
    with open("./Results/GRAY_LBP/GRAY_LBP_star_train.txt", "w") as ftw:
...
def cnn_validation(net, device, loaders, data_size, e, epoch, history):
    ...
    # 結果保存
    with open("./Results/GRAY_LBP/GRAY_LBP_star_validation.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./GRAY_LBP_star__NG_photo_CNN"
...
def cnn_test(net, device, model_path, loaders, data_size):
    ...
    # 結果保存
    with open("./Results/GRAY_LBP/GRAY_LBP_star_test.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./NG_photo_CNN/GRAY_LBP_star_"
    os.makedirs(dir_path, exist_ok=True)
...
def output_graph(epoch, history_train, history_test):
    os.makedirs("./CNNLearningResult/GRAY_LBP_star_", exist_ok=True)
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_LBP_star_/loss_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_LBP_star_/acc_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_LBP_star_/f1_cnn.png')
...
def Main(args):
    ...
    train_loaders, train_data_size = train_load_data("./Features/GRAY_LBP_star_train_cache.pt", ffhq_train_dir, pggan_train_dir, star_train_dir, style_train_dir)
    validation_loaders, validation_data_size = test_load_data("./Features/GRAY_LBP_star_validation_cache.pt", ffhq_validation_dir, pggan_validation_dir, star_validation_dir, style_validation_dir)
    ...
    # 学習済みパラメータを保存
    torch.save(net.state_dict(), './params/GRAY_LBP_star_params_cnn.pth')
    ...
    test_loaders, test_data_size = test_load_data("./Features/GRAY_LBP_star_test_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir, style_test_dir)
    ...
    cnn_test(testnet, device, "./params/GRAY_LBP_star_params_cnn.pth", test_loaders, test_data_size)
