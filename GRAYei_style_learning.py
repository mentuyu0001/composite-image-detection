...
def culc_ei_grayscale(image):
    ...
    # グレースケール画像を無理やり3チャネルに変換
    if image.shape[-1] == 1:  # チャネルが1の場合
        image = np.repeat(image, 3, axis=-1)  # [H, W, 1] -> [H, W, 3] (各チャネルをコピー)
    ...
    for c in range(image.shape[2]):  # チャネル数分繰り返す
        ...
        channel_blurred = cv2.GaussianBlur(channel, (5, 5), 0)  # ノイズ除去のための平滑化
        edges = cv2.Canny(channel_blurred.astype(np.uint8), 50, 150)  # エッジ検出
...
def train_load_data(cache_path, real_dir_path, pggan_dir_path, stargan_dir_path, stylegan_dir_path, target_size=(224, 224)):
    ...
    for img_path in img_path_list:
        ...
        img_tensor = transform(image)  # 画像をテンソル形式に変換 ([C, H, W])

        # 画像特徴をセット
        gray_ei_features.append(culc_ei_grayscale(img_tensor))
    ...
    gray_ei_features = torch.stack(gray_ei_features)  # リストからテンソルに変換
    labels = torch.tensor(labels)
    
    # 画像データ・正解ラベルのペアをデータにセットする
    dataset = torch.utils.data.TensorDataset(gray_ei_features, labels)
...
def cnn_train(net, device, loaders, data_size, optimizer, e, history):
    ...
    for i, (gray_ei_data, labels) in tqdm(enumerate(loaders)):
        # GPUあるいはCPU用に再構成
        gray_ei_data = gray_ei_data.to(device)      # バッチサイズのデータtensor
        ...
        loss_sum += loss.item() # バッチ合計値に直して加算
    ...
    # フォルダがなければ作る
    os.makedirs("./Results/GRAY_EI", exist_ok=True)

    # 結果保存
    with open("./Results/GRAY_EI/GRAY_EI_style_train.txt", "w") as ftw:
...
def cnn_validation(net, device, loaders, data_size, e, epoch, history):
    ...
    with torch.no_grad():
        for i, (gray_ei_data, labels) in enumerate(loaders):
            # GPUあるいはCPU用に再構成
            gray_ei_data = gray_ei_data.to(device)      # バッチサイズのデータtensor
            labels = labels.to(device)  # バッチサイズの正解ラベルtensor
    ...
    # 結果保存
    with open("./Results/GRAY_EI/GRAY_EI_style_validation.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./GRAY_EI_style__NG_photo_CNN"
...
def cnn_test(net, device, model_path, loaders, data_size):
    ...
    # 結果保存
    with open("./Results/GRAY_EI/GRAY_EI_style_test.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./NG_photo_CNN/GRAY_EI_style_"
    os.makedirs(dir_path, exist_ok=True)
...
def output_graph(epoch, history_train, history_test):
    os.makedirs("./CNNLearningResult/GRAY_EI_style_", exist_ok=True)
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_EI_style_/loss_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_EI_style_/acc_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/GRAY_EI_style_/f1_cnn.png')
...
def Main(args):
    ...
    train_loaders, train_data_size = train_load_data("./Features/GRAY_EI_style_train_cache.pt", ffhq_train_dir, pggan_train_dir, star_train_dir, style_train_dir)
    validation_loaders, validation_data_size = test_load_data("./Features/GRAY_EI_style_validation_cache.pt", ffhq_validation_dir, pggan_validation_dir, star_validation_dir, style_validation_dir)
    ...
    # 学習済みパラメータを保存
    torch.save(net.state_dict(), './params/GRAY_EI_style_params_cnn.pth')
    ...
    test_loaders, test_data_size = test_load_data("./Features/GRAY_EI_style_test_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir, style_test_dir)
    ...
    cnn_test(testnet, device, "./params/GRAY_EI_style_params_cnn.pth", test_loaders, test_data_size)
