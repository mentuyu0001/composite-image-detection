...
def cnn_train(net, device, loaders, data_size, optimizer, e, history):
    ...
    # 結果保存
    with open("./Results/DCT/DCT_2class_train.txt", "w") as ftw:
...
def cnn_validation(net, device, loaders, data_size, e, epoch, history):
    ...
    # 結果保存
    with open("./Results/DCT/DCT_2class_validation.txt", "w") as ftw:
...
def last_epoch_NG_output(data, test_pred, target, counter):
    ...
    """
    # フォルダがなければ作る
    dir_path = "./DCT_2class__NG_photo_CNN"
...
def cnn_test(net, device, model_path, loaders, data_size):
    ...
    # 結果保存
    with open("./Results/DCT/DCT_2class_test.txt", "w") as ftw:
...
def output_graph(epoch, history_train, history_test):
    os.makedirs("./CNNLearningResult/DCT_2class_", exist_ok=True)
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/DCT_2class_/loss_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/DCT_2class_/acc_cnn.png')
    ...
    plt.xlabel('epoch')
    plt.legend() # 凡例
    plt.savefig('./CNNLearningResult/DCT_2class_/f1_cnn.png')
...
def Main(args):
    ...
    train_loaders, train_data_size = train_load_data("./Features/DCT_2class_train_cache.pt", ffhq_train_dir, pggan_train_dir, star_train_dir, style_train_dir)
    validation_loaders, validation_data_size = test_load_data("./Features/DCT_2class_validation_cache.pt", ffhq_validation_dir, pggan_validation_dir, star_validation_dir, style_validation_dir)
    ...
    # 学習済みパラメータを保存
    torch.save(net.state_dict(), './params/DCT_2class_params_cnn.pth')
    ...
    test_loaders, test_data_size = test_load_data("./Features/DCT_2class_test_cache.pt", ffhq_test_dir, pggan_test_dir, star_test_dir, style_test_dir)
    ...
    cnn_test(testnet, device, "./params/DCT_2class_params_cnn.pth", test_loaders, test_data_size)
